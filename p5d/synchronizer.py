...
class FileSyncer:
    ...
    def _log_name(self, output_dir: Path, src: Path) -> str:
        ...
        return os.path.join(str(output_dir), f"{os.path.basename(src)}{app_settings.LOG_TEMP_EXT}")
...
class LogMerger:
    ...
    def merge_logs(self) -> None:
        ...
        log_files = [
            f for f in os.listdir(self.output_dir) if f.endswith(app_settings.LOG_TEMP_EXT)
        ]
