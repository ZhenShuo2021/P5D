...
from src.categorizer import CategorizerUI, ConfigLoader, LogManager, CategorizerFactory

class LoggingTestCase(unittest.TestCase):
    def setUp(self):
        super().setUp()
        self.mock_logger = Mock(spec=LogManager)
...
class TestCategorizerUI(LoggingTestCase):
    def setUp(self):
        super().setUp()
        ...
        self.mock_config_loader.get_base_paths.return_value = {"local_path": "/mock/path"}
        self.mock_config_loader.get_combined_paths.return_value = {"path1": "/mock/path1", "path2": "/mock/path2"}
        self.mock_config_loader.get_categories.return_value = {self.test_cat1, self.test_cat2}

    @patch('pathlib.Path.exists', return_value=True)
    def test_init_success(self, mock_exists):
        ui = CategorizerUI(self.mock_config_loader, self.mock_logger, self.mock_factory)
        self.assertIsInstance(ui, CategorizerUI)

    @patch('pathlib.Path.exists', return_value=False)
    def test_init_failure(self, mock_exists):
        with self.assertRaises(FileNotFoundError):
            CategorizerUI(self.mock_config_loader, self.mock_logger, self.mock_factory)
    ...
    def test_categorize_all(self, mock_exists):
        ...
        self.assertEqual(self.mock_factory.get_categorizer.call_count, len(self.mock_config_loader.get_categories.return_value))
        self.assertEqual(mock_categorizer.categorize.call_count, len(self.mock_config_loader.get_categories.return_value))
